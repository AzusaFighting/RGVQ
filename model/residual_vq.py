...
class ResidualVQ(Module):
    ...
    def forward(
        ...
        for quantizer_index, (vq, maybe_mlp) in enumerate(zip(self.layers, maybe_code_transforms)):
            ...
            quantized, embed_indices, loss, orig_quantize, soft_weights = vq(
                residual,
                mask = mask,
        ...
        all_losses, all_indices, all_soft = map(partial(torch.stack, dim = -1), (all_losses, all_indices, all_soft))

        ret = (quantized_out, all_indices, all_losses, all_soft)
