...
#收集distributed samples from every GPU and return their union
def sample_vectors_distributed(local_samples, num):
    local_samples = rearrange(local_samples, '1 ... -> ...')

    rank = distributed.get_rank()
    all_num_samples = all_gather_sizes(local_samples, dim = 0) #[sample_num at GPU1, num at GPU2, ..., num at GPUn]
...
def kmeans(
    ...
    num_codebooks, dim, dtype, device = samples.shape[0], samples.shape[-1], samples.dtype, samples.device #shape[0]=n_head?
...
class EuclideanCodebook(nn.Module):
    ...
    def forward(
        ...
        # embed is the center of learned codebook
        ...
        embed_ind, embed_onehot, soft_weights = self.gumbel_sample(dist, dim = -1, temperature = sample_codebook_temp, training = self.training)
        ...
        return quantize, embed_ind, dist, soft_weights
...
class CosineSimCodebook(nn.Module):
    ...
    def forward(
        ...
        if self.training and not freeze_codebook:
            ...
            bins = embed_onehot.sum(dim = 1) #(h, sum, c)
            ...
            ema_inplace(self.embed_avg.data, embed_sum, self.decay) #EMA update updated = decay * old + (1-decay) * new
...
class VectorQuantize(nn.Module):
    def __init__(
        ...
        affine_param = False,
        affine_param_batch_decay = 0.9,
        affine_param_codebook_decay = 0.99,
        ...
        assert not (ema_update and learnable_codebook), 'learnable codebook not compatible with EMA update 冲突了'
    ...
    def load_code(self, learned_codes):
        if learned_codes.ndim == 3:
            codes = learned_codes.reshape(-1, learned_codes.shape[-1])
        self.learned_codebook = nn.Embedding.from_pretrained(codes.clone(), freeze=False)
    ...
    def forward(
        ...
        quantize, embed_ind, distances, soft_weights = self._codebook(x, **codebook_forward_kwargs)
        ...
        return quantize, embed_ind, loss, orig_quantize, soft_weights